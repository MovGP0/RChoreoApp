...
impl Default for OutlineButton<'_> {
    fn default() -> Self {
        Self::new("")
    }
}
...
mod tests {
    ...
    use super::OutlineButton;

    #[test]
    fn outline_button_renders_without_panicking() {
        let context = Context::default();
        let mut min_height = 0.0;
        let _ = context.run(egui::RawInput::default(), |ctx| {
            egui::CentralPanel::default().show(ctx, |ui| {
                let response = OutlineButton::new("Outline").show(ui);
