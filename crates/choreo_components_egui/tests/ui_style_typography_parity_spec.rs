...
use choreo_components_egui::ui_style::typography::platform_font_fallback_chain;
use choreo_components_egui::ui_style::typography::style_for_role;
...
fn typography_declares_platform_safe_fallback_chain() {
    assert_eq!(
        ...
        &["Roboto", "Segoe UI", "Noto Sans", "Helvetica Neue", "Arial", "sans-serif"]
    );
}
...
fn typography_roles_are_assigned_for_primary_surfaces() {
    ...
    assert_eq!(settings_ui::page_title_role(), TypographyRole::HeadlineSmall);
    assert_eq!(settings_ui::section_label_role(), TypographyRole::TitleSmall);
    ...
    assert_eq!(scenes_ui::scene_title_role(), TypographyRole::TitleSmall);
    assert_eq!(scenes_ui::scene_timestamp_role(), TypographyRole::BodySmall);
}

fn assert_style(role: TypographyRole, expected_size_px: f32, expected_line_height_px: f32, expected_weight: i32) {
