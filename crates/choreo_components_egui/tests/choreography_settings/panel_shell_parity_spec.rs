...
use choreo_components_egui::choreography_settings::ui::uses_vertical_scroll_container;
...
fn render_debug_shapes(
    state: &choreo_components_egui::choreography_settings::state::ChoreographySettingsState,
) -> String {
    let context = egui::Context::default();
    let output = context.run(egui::RawInput::default(), |ctx| {
        egui::CentralPanel::default().show(ctx, |ui| {
            let _ = choreo_components_egui::choreography_settings::ui::draw(ui, state);
        });
    });
