...
fn ui_style_tokens_spec() {
    let suite = rspec::describe("ui style metrics token parity", (), |spec| {
        spec.it("maps source Slint size, padding, and corner-radius tokens", |_| {
        ...
        spec.it("dialog host consumes shared dialog metrics tokens", |_| {
            ...
            assert_eq!(dialog_metrics.dialog_padding, metrics.paddings.padding_24 as i8);
        ...
        spec.it("component UIs consume shared spacing/radius/state-opacity tokens", |_| {
            let metrics = material_style_metrics();
            ...
            assert!((state_opacity_tokens.disabled - metrics.state_opacities.content_disabled).abs() < f32::EPSILON);
            assert!((state_opacity_tokens.hover - metrics.state_opacities.hover).abs() < f32::EPSILON);
            assert!((state_opacity_tokens.pressed - metrics.state_opacities.pressed).abs() < f32::EPSILON);
        });
    });
