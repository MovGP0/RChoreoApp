...
/// current crate and are kept separate to avoid breaking existing callers.
...
pub const MATERIAL_STYLE_METRICS: MaterialStyleMetrics = MaterialStyleMetrics {
    // Slint source order: sizes, icon, padding, spacing, border_radius.
    sizes: slint_size_tokens(),
    icon_sizes: slint_icon_size_tokens(),
    paddings: slint_padding_tokens(),
    spacings: slint_spacing_tokens(),
    corner_radii: slint_corner_radius_tokens(),
    // egui-only extensions preserved for current callers.
    strokes: egui_stroke_tokens(),
    elevations: egui_elevation_tokens(),
    state_opacities: egui_state_opacity_tokens(),
};

#[must_use]
pub const fn material_style_metrics() -> MaterialStyleMetrics {
    MATERIAL_STYLE_METRICS
}

const fn slint_size_tokens() -> SizeTokens {
    SizeTokens {
        size_1: 1.0,
        size_2: 2.0,
        size_3: 3.0,
        ...
        size_72: 72.0,
        size_80: 80.0,
        // The Slint source keeps the legacy token name while assigning 96px.
        size_90: 96.0,
        size_200: 200.0,
        size_256: 256.0,
        size_344: 344.0,
...
const fn slint_icon_size_tokens() -> IconSizeTokens {
    IconSizeTokens {
        icon_size_18: 18.0,
        icon_size_24: 24.0,
        icon_size_36: 36.0,
        // The Slint source keeps the legacy token name while assigning 27px.
        icon_size_90: 27.0,
    }
}

const fn slint_padding_tokens() -> PaddingTokens {
    PaddingTokens {
        padding_4: 4.0,
        padding_6: 6.0,
        padding_10: 10.0,
...
const fn slint_spacing_tokens() -> SpacingTokens {
    SpacingTokens {
        spacing_2: 2.0,
        spacing_6: 6.0,
        spacing_4: 4.0,
...
const fn slint_corner_radius_tokens() -> CornerRadiusTokens {
    CornerRadiusTokens {
        border_radius_2: 2.0,
        border_radius_4: 4.0,
        border_radius_8: 8.0,
...
const fn egui_stroke_tokens() -> StrokeTokens {
    StrokeTokens {
        outline: 1.0,
        focus: 2.0,
    }
...
const fn egui_elevation_tokens() -> ElevationTokens {
    ElevationTokens {
...
const fn egui_state_opacity_tokens() -> StateOpacityTokens {
    StateOpacityTokens {
        hover: 0.08,
        focus: 0.10,
        pressed: 0.10,
