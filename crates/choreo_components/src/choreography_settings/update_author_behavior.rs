...
use super::messages::{RedrawFloorCommand, UpdateAuthorCommand};
...
pub struct UpdateAuthorBehavior {
    ...
    receiver: Option<Receiver<UpdateAuthorCommand>>,
}
...
impl UpdateAuthorBehavior {
    ...
    pub fn new_with_receiver(
        global_state: Rc<GlobalStateActor>,
        redraw_sender: Sender<RedrawFloorCommand>,
        receiver: Receiver<UpdateAuthorCommand>,
    ) -> Self {
        Self {
            global_state,
            redraw_sender,
    ...
    fn update_author(&self, value: &str) {
    ...
    fn activate(
        ...
        timer.start(TimerMode::Repeated, Duration::from_millis(16), move || {
            while let Ok(command) = receiver.try_recv() {
                behavior.update_author(&command.value);
            }
        });
