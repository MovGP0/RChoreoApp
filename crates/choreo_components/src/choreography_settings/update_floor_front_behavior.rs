...
use super::messages::{RedrawFloorCommand, UpdateFloorFrontCommand};
...
pub struct UpdateFloorFrontBehavior {
    ...
    receiver: Option<Receiver<UpdateFloorFrontCommand>>,
}
...
impl UpdateFloorFrontBehavior {
    ...
    pub fn new_with_receiver(
        global_state: Rc<GlobalStateActor>,
        redraw_sender: Sender<RedrawFloorCommand>,
        receiver: Receiver<UpdateFloorFrontCommand>,
    ) -> Self {
        Self {
            global_state,
            redraw_sender,
    ...
    fn update_floor_front(&self, value: i32) {
    ...
    fn activate(
        ...
        timer.start(TimerMode::Repeated, Duration::from_millis(16), move || {
            while let Ok(command) = receiver.try_recv() {
                behavior.update_floor_front(command.value);
            }
        });
