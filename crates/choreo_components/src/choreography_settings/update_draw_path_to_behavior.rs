...
use super::messages::{RedrawFloorCommand, UpdateDrawPathToCommand};
use nject::injectable;

#[injectable]
pub struct UpdateDrawPathToBehavior<P: Preferences + Clone> {
    preferences: P,
    redraw_sender: Sender<RedrawFloorCommand>,
    receiver: Option<Receiver<UpdateDrawPathToCommand>>,
}

impl<P: Preferences + Clone> UpdateDrawPathToBehavior<P> {
    ...
    pub fn new_with_receiver(
        preferences: P,
        redraw_sender: Sender<RedrawFloorCommand>,
        receiver: Receiver<UpdateDrawPathToCommand>,
    ) -> Self {
        Self {
            preferences,
            redraw_sender,
    ...
    fn update_draw_path_to(&self, view_model: &mut ChoreographySettingsViewModel, value: bool) {
    ...
    fn activate(
        ...
        timer.start(TimerMode::Repeated, Duration::from_millis(16), move || {
            while let Ok(command) = receiver.try_recv() {
                ...
                behavior.update_draw_path_to(&mut view_model, command.value);
