...
fn run_in_ui_thread(test: impl FnOnce() + Send + 'static) {
    let handle = std::thread::Builder::new()
        .name("navigate-settings-to-main-spec".to_string())
...
fn navigate_settings_to_main_spec() {
    let suite = rspec::describe("navigate from settings page to main page", (), |spec| {
        spec.it("returns to the main page when settings requests back navigation", |_| {
            run_in_ui_thread(|| {
