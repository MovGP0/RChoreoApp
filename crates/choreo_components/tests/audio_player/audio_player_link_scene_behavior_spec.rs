...
fn audio_player_link_scene_behavior_spec() {
    let suite = rspec::describe("audio player link scene behavior", (), |spec| {
        spec.it(
            "links selected scene timestamp when position is between neighbors",
            |_| {
                ...
                let behavior =
                    AudioPlayerLinkSceneBehavior::new(global_state.clone(), link_receiver);
                let context = audio_player::AudioPlayerTestContext::with_global_state(
                ...
                let model_timestamp = context
                    .read_global_state(|state| state.choreography.scenes[1].timestamp.clone());
