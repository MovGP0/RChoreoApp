...
fn setup_context() -> (common::FloorTestContext, RotateAroundCenterBehavior) {
...
fn rotate_around_center_behavior_spec() {
    let suite = rspec::describe("rotate around center behavior", (), |spec| {
        spec.it("rotates selected positions around center", |_| {
            ...
            common::assert_close(first.x, 0.0, 0.0001);
            common::assert_close(first.y, 2.0, 0.0001);
            common::assert_close(second.x, 0.0, 0.0001);
            common::assert_close(second.y, 0.0, 0.0001);
