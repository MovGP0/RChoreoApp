...
fn color_preferences_behavior_spec() {
    let suite = rspec::describe("color preferences behavior", (), |spec| {
        spec.it("enforces primary-secondary-tertiary enablement hierarchy", |_| {
            ...
            context.view_model.borrow_mut().update_use_secondary_color(true);
            context.pump_events();
            assert!(!context.view_model.borrow().use_secondary_color);

            context.view_model.borrow_mut().update_use_primary_color(true);
            ...
            context.view_model.borrow_mut().update_use_secondary_color(true);
            context.view_model.borrow_mut().update_use_tertiary_color(true);
            ...
            context.view_model.borrow_mut().update_use_primary_color(false);
