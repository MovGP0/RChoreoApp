...
fn save_choreo_behavior_spec() {
    let suite = rspec::describe("save choreo behavior", (), |spec| {
        spec.it("saves current scenes to the last opened choreography file", |_| {
            ...
            let source_scene = scenes::build_scene_model(1, "Intro", Some("00:12"), vec![scenes::build_position(1.0, 2.0)]);
            ...
            let behavior = SaveChoreoBehavior::new(context.global_state_store.clone(), preferences_dyn);
            context.activate_behaviors(vec![Box::new(behavior) as Box<dyn Behavior<_>>]);

            context.view_model.borrow_mut().save_choreo();
            context.pump_events();

            let saved_json = fs::read_to_string(&choreo_path).expect("saved file should be readable");
            let saved = choreo_master_mobile_json::import(&saved_json).expect("saved file should be valid choreography json");
        ...
        spec.it("does nothing when last opened file path is missing", |_| {
            ...
            assert_eq!(context.read_global_state(|state| state.choreography.name.clone()), original_name);
        });
    });
