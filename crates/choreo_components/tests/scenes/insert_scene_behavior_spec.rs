...
fn insert_scene_behavior_spec() {
    let suite = rspec::describe("insert scene behavior", (), |spec| {
        spec.it("inserts a scene after the selected one and selects it", |_| {
            ...
            let selected = context.read_global_state(|state| state.selected_scene.clone()).expect("selected scene should exist");
            assert_eq!(selected.scene_id, scenes[1].scene_id);
            assert_eq!(context.read_global_state(|state| state.choreography.scenes.len()), 3);
        });
        ...
        spec.it("inserts at the end when no scene is selected", |_| {
            ...
            assert_eq!(scenes[1].name, "New Scene");
            assert_eq!(context.read_global_state(|state| state.choreography.scenes.len()), 2);
        });
    });
