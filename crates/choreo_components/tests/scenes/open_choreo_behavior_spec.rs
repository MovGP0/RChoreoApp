...
fn open_choreo_behavior_spec() {
    let suite = rspec::describe("open choreo behavior", (), |spec| {
        spec.it("loads choreography from contents and triggers scene/audio updates", |_| {
            ...
            let json = build_choreo_json("Imported", true, Some(audio_path.to_string_lossy().into_owned()));
            ...
            let preferences_dyn = context.preferences.clone() as Rc<dyn choreo_components::preferences::Preferences>;
        ...
        spec.it("closes audio and stores file name when importing without file path", |_| {
            let context = scenes::ScenesTestContext::new();
