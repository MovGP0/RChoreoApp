...
use common::Report;
use choreo_components::floor::{Point, ScalePositionsBehavior};
use choreo_components::global::InteractionMode;
use choreo_state_machine::ScalePositionsStartedTrigger;

fn setup_context() -> (common::FloorTestContext, ScalePositionsBehavior) {
    ...
    context.global_state.interaction_mode = InteractionMode::Scale;
    let _ = context.state_machine.try_apply(&ScalePositionsStartedTrigger);

    (context, ScalePositionsBehavior::default())
}

fn select_rectangle(
    behavior: &mut ScalePositionsBehavior,
...
fn scale_positions_behavior_spec() {
    let suite = rspec::describe("scale positions behavior", (), |spec| {
        spec.it("scales selected positions", |_| {
            ...
            drag_from_to(&mut behavior, &mut context, Point::new(2.0, 1.0), Point::new(4.0, 1.0));
            ...
            common::assert_close(first.x, -2.0, 0.0001);
            common::assert_close(first.y, 1.0, 0.0001);
            common::assert_close(second.x, 2.0, 0.0001);
