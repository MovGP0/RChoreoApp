...
- View: functions that build egui UI (`fn ui(&mut self, ui: &mut egui::Ui, model: &Model) -> Vec<Action>`) or directly mutate via commands.
- Update/Reducer: one place that applies actions/events to the model.

Why it fits egui: egui already produces per-frame events; convert them into domain actions and apply them.
...
Then in your `App::update(ctx, frame)`:
...
This maps cleanly to multi-touch and to your `wgpu` scene.
