...
- Behavior activation and tracing gates already have centralized hooks in `crates/choreo_components/src/observability/mod.rs`.
- UI actions fan out through `MainPageBinding` and the behavior pipeline in `crates/choreo_components/src/choreo_main/`.
