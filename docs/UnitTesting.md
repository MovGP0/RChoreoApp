...
- Every spec wraps scenarios in a single `rspec::describe("<behavior>", (), |spec| { ... })` suite and executes it with `floor::run_suite(&suite)` from `crates/choreo_components/tests/floor/mod.rs`.
