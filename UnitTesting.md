...
- Existing floor integration tests use one spec file per behavior in `crates/choreo_components_egui/tests/floor/*_spec.rs`.
- Every spec wraps scenarios in a single `rspec::describe("<behavior>", (), |spec| { ... })` suite and executes it with `floor::run_suite(&suite)` from `crates/choreo_components_egui/tests/floor/mod.rs`.
- The outer Rust test function name matches the file purpose (example: `move_positions_behavior_spec`) and is marked with `#[serial_test::serial]` to avoid shared UI timing interference.
